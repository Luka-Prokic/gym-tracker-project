...
import { ExerciseWidget } from "../gym/ExerciseWidget";
import { SuperSetWidget } from "../gym/SuperSetWidget";
import { CardioWidget } from "../gym/CardioWidget";
import ISlide from "../bubbles/ISlide";
import { ExerciseSwapList } from "../gym/ExerciseSwapList";
import useBubbleLayout from "../bubbles/hooks/useBubbleLayout";
import { GymTable } from "../gym/gym_table/GymTable";
import { SuperSetTable } from "../gym/superset_table/SuperSetTable";

const MAX_VISIBLE_CELLS = 5;
const CELL_HEIGHT = 44;

const LayoutExerciseBar: React.FC = React.memo(() => {
    ...
    // Early return if no active routine
    if (!activeRoutine) {
        return null;
    }
                            ...
                            >
                                ...
                                <View style={styles.grid}>
                                    {layout.map((ex: LayoutItem, index: number) => {
                                        ...
                                        if (isGymExercise(ex)) {
                                            return <ExerciseWidget {...commonProps} />;
                                        }
                                        if (isSuperSet(ex)) {
                                            return <SuperSetWidget {...commonProps} />;
                                        }
                                        if (isCardioExercise(ex)) {
                                            return <CardioWidget {...commonProps} />;
                                        }
                                        return null;
                                    })}
                                </View>
...
export default LayoutExerciseBar;
